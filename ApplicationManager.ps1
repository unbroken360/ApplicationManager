...
Function Create-IntuneApp {  
    ...
    if (!$IntuneApp) {      
        ...
        if ($DetectionType -eq 'Script') {
            # Create PowerShell script file for detection
            $ScriptFilePath = "$($IntuneOutputFolder)\$($DisplayName)-Detection.ps1"
            Set-Content -Path $ScriptFilePath -Value $DetectionMethod

            # Create detection rule using PowerShell script
            $DetectionRule = New-IntuneWin32AppDetectionRuleScript -ScriptFile $ScriptFilePath -EnforceSignatureCheck $false -RunAs32Bit $false
        }
        elseif ($TextBoxMSIPackage.Text.Length -gt 0) {
...
function Create-ApplicationObjects {
    ...
    if ($CreateInIntune) {
        ...
        $IntuneApp = Create-IntuneApp -Publisher $Publisher -DisplayName $ApplicationName -ContentSourcePath $ContentSourcePath -InstallationProgram $InstallationProgram -UninstallationProgram $UninstallationProgram -Description $ApplicationDescription -DetectionType $DetectionType -DetectionMethod $DetectionMethod
